...
The time series support in Flot is based on Javascript Dates,
i.e. everywhere a time value is expected or handed over, a Javascript
Date number is used. This is a number, not a Date object. A
Javascript Date is the number of milliseconds since January 1,
1970 00:00:00 UTC. This is almost the same as Unix Dates, except it's
in milliseconds, so remember to multiply by 1000!

You can see a Date like this
...
Dates. Many will want the Dates to be displayed according to
a certain time zone, usually the time zone in which the data has been
produced. Some want the localized experience, where the Dates are
...
Default behavior is that Flot always displays Dates according to
...
datapoint at 2002-02-20 08:00, you can generate a Date for eight
o'clock UTC even if it really happened eight o'clock UTC+0200.

In PHP you can get an appropriate Date with:
...
```aspx
public static int GetJavascriptDate(System.DateTime input)
{
...
possible to generate the Dates manually client-side.

If you've already got the real UTC Date, it's too late to use the
pretend trick described above. But you can fix up the Dates by
adding the time zone offset, e.g. for UTC+0200 you would add 2 hours
to the UTC Date you got. Then it'll look right on the plot. Most
programming environments have some means of getting the timezone
offset for a specific date (note that you need to get the offset for
each individual Date to account for daylight savings).

The alternative with core Javascript is to interpret the Dates
...
interpret the Dates according to that time zone.

Once you've gotten the Dates into the data and specified "time"
as the axis mode, Flot will automatically generate relevant ticks and
format them. As always, you can tweak the ticks via the "ticks" option
- just remember that the values should be Dates (numbers), not
Date objects.
...
If you set "twelveHourClock" to true, the autogenerated Dates
will use 12 hour AM/PM Dates instead of 24 hour. This only
